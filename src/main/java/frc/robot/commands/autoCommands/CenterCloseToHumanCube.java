...
public class CenterCloseToHumanCube extends SequentialCommandGroup {
  ...
  collectWheelsSubsystem collectWheels, shootingSubsystem shootingSubsystem, armCollectSubsystem armCollectSubsystem
    ...
    // Previos:
    // addCommands(new ParallelDeadlineGroup(new WaitCommand(14.85), new SequentialCommandGroup(
    //   new InstantCommand(() -> swerve.zeroGyro()), new ShootingCommand(shootingSubsystem, cartridgeSubsystem, armCollectSubsystem, 0.75, 0.3),
    // new StartAuto(AutoCommand.getAutoCommand(swerve, "center - start Close To Human Cube", 3), armCollectSubsystem, swerve),
    // new moveInParallel(swerve, collectSubsystem, collectWheels, armCollectSubsystem, cartridgeSubsystem, AutoCommand.getAutoCommand(swerve, "center - Close To Human Cube", 3), Constants.COLLECT_OPEN_POSITION, Constants.ARM_OPEN_POSITION, 2, 0.5, false),
